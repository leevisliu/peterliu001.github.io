(function(exports, Laya) {
    ...
    class Widget extends Laya.Component {
        ...
        _onEnable() {
            ...
            else this.owner.once(Laya.Event.ADDED, this, this._onAdded);
        }
        _onDisable() {
        ...
        resetLayoutX() {
            ...
            if (parent) {
                if (!isNaN(this.centerX)) {
                    owner.x = Math.round((parent.width - owner.displayWidth) * .5 + this.centerX + owner.pivotX * owner.scaleX);
        ...
        resetLayoutY() {
            ...
            if (parent) {
                if (!isNaN(this.centerY)) {
                    owner.y = Math.round((parent.height - owner.displayHeight) * .5 + this.centerY + owner.pivotY * owner.scaleY);
    ...
    UIUtils.grayFilter = new Laya.ColorFilter([.3086, .6094, .082, 0, 0, .3086, .6094, .082, 0, 0, .3086, .6094, .082, 0, 0, 0, 0, 0, 1, 0]);
    ...
    class Image extends UIComponent {
        ...
        set width(value) {
            super.width = value;
            this._bitmap.width = value == 0 ? 1e-7 : value;
        }
        get width() {
            return super.width;
        }
        set height(value) {
            super.height = value;
            this._bitmap.height = value == 0 ? 1e-7 : value;
        }
        get height() {
            return super.height;
        }
    ...
    class AdvImage extends Image {
        ...
        onGetAdvsListData() {
            var _this = this;
            var random = AdvImage.randRange(1e4, 1e6);
    ...
    class CheckBox extends Button {
        ...
        set dataSource(value) {
            this._dataSource = value;
            if (value instanceof Boolean) this.selected = value;
    ...
    class Clip extends UIComponent {
        ...
        _onDisplay(e) {
            if (this._isPlaying) {
                if (this._getBit(Laya.Const.DISPLAYED_INSTAGE)) this.play();
                else this.stop();
        ...
        set dataSource(value) {
            this._dataSource = value;
            if (typeof value == "number" || typeof value == "string") this.index = parseInt(value);
            else super.dataSource = value;
        }
        get dataSource() {
            return super.dataSource;
    ...
    class ColorPicker extends UIComponent {
        ...
        changePanel() {
            ...
            g.clear(true);
            var mainColors = [0, 3355443, 6710886, 10066329, 13421772, 16777215, 16711680, 65280, 255, 16776960, 65535, 16711935];
            for (var i = 0; i < 12; i++) {
                for (var j = 0; j < 20; j++) {
                    var color;
                    if (j === 0) color = mainColors[i];
                    else if (j === 1) color = 0;
                    else color = (((i * 3 + j / 6) % 3 << 0) + (i / 6 << 0) * 3) * 51 << 16 | j % 6 * 51 << 8 | (i << 0) % 6 * 51;
    ...
    class Slider extends UIComponent {
        ...
        showValueText() {
            if (this.showLabel) {
                ...
                if (this.isVertical) {
                    ...
                    label.y = (this._bar.height - label.height) * .5 + this._bar._y;
                } else {
                    label.y = this._bar._y - 20;
                    label.x = (this._bar.width - label.width) * .5 + this._bar._x;
                }
            }
        }
        hideValueText() {
        ...
        mouseMove(e) {
            ...
            if (this.isVertical) {
                ...
                if (this._progress) this._progress.height = this._bar._y + .5 * this._bar.height;
            } else {
                ...
                if (this._progress) this._progress.width = this._bar._x + .5 * this._bar.width;
            }
        ...
        set allowClickBack(value) {
            if (this._allowClickBack != value) {
                ...
                if (value) this._bg.on(Laya.Event.MOUSE_DOWN, this, this.onBgMouseDown);
        ...
        tweenMove(maxDistance) {
            ...
            if (maxDistance > 0) {
                if (this._lastOffset > 0 && this.value <= this.min) {
                    this._isElastic = true;
                    tarSpeed = -(this.min - maxDistance - this.value) * .5;
                    if (this._lastOffset > tarSpeed) this._lastOffset = tarSpeed;
                } else if (this._lastOffset < 0 && this.value >= this.max) {
                    this._isElastic = true;
                    tarSpeed = -(this.max + maxDistance - this.value) * .5;
                    if (this._lastOffset < tarSpeed) this._lastOffset = tarSpeed;
                }
            }
            this.value -= this._lastOffset;
    ...
    class List extends Box {
        ...
        set cacheAs(value) {
            ...
            if (this._scrollBar) {
                this._usedCache = null;
                if (value !== "none") this._scrollBar.on(Laya.Event.START, this, this.onScrollStart);
                else this._scrollBar.off(Laya.Event.START, this, this.onScrollStart);
            }
        }
        get cacheAs() {
        ...
        set dataSource(value) {
            this._dataSource = value;
            if (typeof value == "number" || typeof value == "string") this.selectedIndex = parseInt(value);
    ...
    class TextArea extends TextInput {
        ...
        changeScroll() {
            ...
            if (this._hScrollBar) {
                ...
                this._hScrollBar.scrollSize = Math.max(showWidth * .033, 1);
    ...
    class Panel extends Box {
        ...
        changeScroll() {
            ...
            if (vscroll) {
                ...
                vscroll.scrollSize = Math.max(this._height * .033, 1);
            ...
            if (hscroll) {
                ...
                hscroll.scrollSize = Math.max(this._width * .033, 1);
        ...
        changeItems() {
            ...
            for (i = 0, n = items.length; i < n; i++) {
                ...
                if (this._align == HBox.TOP) {
                    ...
                    item.y = (maxHeight - item.height * item.scaleY) * .5;
        ...
        changeItems() {
            ...
            for (i = 0, n = items.length; i < n; i++) {
                ...
                if (this._align == VBox.LEFT) {
                    ...
                    item.x = (maxWidth - item.width * item.scaleX) * .5;
